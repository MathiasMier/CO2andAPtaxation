...
dref_int(r,t)            Indexed reference growth path
...
$gdxin database\setpar_%n%.gdx
$load hours, load_s, loadcorr, peakload=peakload_s, minload=minload_s, dref_int=dref, daref, daref_h, daref_s
$load vrsc_s, vrsccorr, irnwflh_h, irnwflh_s, number
$gdxin
...
drefval = 0 ;
$if set load05  drefval = 0.5 ;
$if set load04  drefval = 0.4 ;
$if set load03  drefval = 0.3 ;
$if set load02  drefval = 0.2 ;
$if set load01  drefval = 0.1 ;

drefadj(t) = 0 + (0 + drefval) * (t.val - 2020) / (2050 - 2020) ;
drefadj(t) = 0 + (0 + drefval) * (t.val - 2020) / (2050 - 2020) ;

dref(r,t) = round(dref_int(r,t) / (1 + drefadj(t)), 4) ;

daref_check(r,t)   = sum(s, hours(s) * load(s,r)) * dref(r,t) ;
daref(r,t)         = daref_check(r,t) * 1e-3 ;
...
$gdxin random_readin_set.gdx   
$load typ, maptyp
$gdxin


parameter
capcostadj_typ(typ,draw)
capcostadj(i)
;

$gdxin random_readin_sigma.gdx    
$if not set sigma02 $load capcostadj_typ = capcostadj01_typ
$if     set sigma02 $load capcostadj_typ = capcostadj02_typ
$gdxin

capcostadj(i) = 1 ;
$if set draw1    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"1")) ;
$if set draw2    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"2")) ;
$if set draw3    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"3")) ;
$if set draw4    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"4")) ;
$if set draw5    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"5")) ;
$if set draw6    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"6")) ;
$if set draw7    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"7")) ;
$if set draw8    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"8")) ;
$if set draw9    capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"9")) ;
$if set draw10   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"10")) ;
$if set draw11   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"11")) ;
$if set draw12   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"12")) ;
$if set draw13   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"13")) ;
$if set draw14   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"14")) ;
$if set draw15   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"15")) ;
$if set draw16   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"16")) ;
$if set draw17   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"17")) ;
$if set draw18   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"18")) ;
$if set draw19   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"19")) ;
$if set draw20   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"20")) ;
$if set draw21   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"21")) ;
$if set draw22   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"22")) ;
$if set draw23   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"23")) ;
$if set draw24   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"24")) ;
$if set draw25   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"25")) ;
$if set draw26   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"26")) ;
$if set draw27   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"27")) ;
$if set draw28   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"28")) ;
$if set draw29   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"29")) ;
$if set draw30   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"30")) ;
$if set draw31   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"31")) ;
$if set draw32   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"32")) ;
$if set draw33   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"33")) ;
$if set draw34   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"34")) ;
$if set draw35   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"35")) ;
$if set draw36   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"36")) ;
$if set draw37   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"37")) ;
$if set draw38   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"38")) ;
$if set draw39   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"39")) ;
$if set draw40   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"40")) ;
$if set draw41   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"41")) ;
$if set draw42   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"42")) ;
$if set draw43   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"43")) ;
$if set draw44   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"44")) ;
$if set draw45   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"45")) ;
$if set draw46   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"46")) ;
$if set draw47   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"47")) ;
$if set draw48   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"48")) ;
$if set draw49   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"49")) ;
$if set draw50   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"50")) ;
$if set draw51   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"51")) ;
$if set draw52   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"52")) ;
$if set draw53   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"53")) ;
$if set draw54   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"54")) ;
$if set draw55   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"55")) ;
$if set draw56   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"56")) ;
$if set draw57   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"57")) ;
$if set draw58   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"58")) ;
$if set draw59   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"59")) ;
$if set draw60   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"60")) ;
$if set draw61   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"61")) ;
$if set draw62   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"62")) ;
$if set draw63   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"63")) ;
$if set draw64   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"64")) ;
$if set draw65   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"65")) ;
$if set draw66   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"66")) ;
$if set draw67   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"67")) ;
$if set draw68   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"68")) ;
$if set draw69   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"69")) ;
$if set draw70   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"70")) ;
$if set draw71   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"71")) ;
$if set draw72   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"72")) ;
$if set draw73   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"73")) ;
$if set draw74   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"74")) ;
$if set draw75   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"75")) ;
$if set draw76   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"76")) ;
$if set draw77   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"77")) ;
$if set draw78   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"78")) ;
$if set draw79   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"79")) ;
$if set draw80   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"80")) ;
$if set draw81   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"81")) ;
$if set draw82   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"82")) ;
$if set draw83   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"83")) ;
$if set draw84   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"84")) ;
$if set draw85   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"85")) ;
$if set draw86   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"86")) ;
$if set draw87   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"87")) ;
$if set draw88   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"88")) ;
$if set draw89   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"89")) ;
$if set draw90   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"90")) ;
$if set draw91   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"91")) ;
$if set draw92   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"92")) ;
$if set draw93   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"93")) ;
$if set draw94   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"94")) ;
$if set draw95   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"95")) ;
$if set draw96   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"96")) ;
$if set draw97   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"97")) ;
$if set draw98   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"98")) ;
$if set draw99   capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"99")) ;
$if set draw100  capcostadj(i) = sum(maptyp(i,typ), capcostadj_typ(typ,"100")) ;
...
capcostadj_v(i,"2015") = 1 ;
capcostadj_v(i,"2050") = 1 ;

capcostadj_v(wind(i),"2050") = capcostadj(i) ;
capcostadj_v(ccs(i),"2050") = capcostadj(i) ;
capcostadj_v(sol(i),"2050") = capcostadj(i) ;
capcostadj_v(nuc(i),"2050") = capcostadj(i) ;

capcostadj_v(i,newv(v))$(not sameas(v,"2015") or not sameas(v,"2050")) = capcostadj_v(i,"2015") + (capcostadj_v(i,"2050") - capcostadj_v(i,"2015")) * (v.val - 2015) / (2050 - 2015) ;
capcost_int(i,v,r) = capcost(i,v,r) ;
capcost(i,newv(v),r) = capcost_int(i,v,r) * capcostadj_v(i,v) ;
...
$if      set myopic $if not set nodisc   $if set overlap8        $load endeffect=modeldepr
$if      set myopic $if set nodisc       $if set overlap8        $load endeffect=modeldepr_nodisc
...
$if      set myopic $if not set nodisc   $if set overlap8        $load gendeffect=gmodeldepr
$if      set myopic $if set nodisc       $if set overlap8        $load gendeffect=gmodeldepr_nodisc
...
$if      set myopic $if not set nodisc   $if set overlap8        $load tendeffect=tmodeldepr
$if      set myopic $if set nodisc       $if set overlap8        $load tendeffect=tmodeldepr_nodisc
$gdxin
...
$if      set scapequal           scapr(ap,impactap,t)                    = sum(r, daref(r,t) * scap_i(ap,impactap,r,t))                  / sum(r, daref(r,t)) ;
$if      set scapequal           scapr_emit_impactap(impactap,i,v,t)     = sum(r, daref(r,t) * scap_emit_impactap_i(impactap,i,v,r,t))   / sum(r, daref(r,t)) ;
$if      set scapequal           scapr_emit_ap(ap,i,v,t)                 = sum(r, daref(r,t) * scap_emit_ap_i(ap,i,v,r,t))               / sum(r, daref(r,t)) ;
$if      set scapequal           scapr_emit(i,v,t)                       = sum(r, daref(r,t) * scap_emit_i(i,v,r,t))                     / sum(r, daref(r,t)) ;
...
$if      set scapequaldistri     scapr(ap,impactap,t)                    = sum(r, daref(r,t) * scap_i(ap,impactap,r,t))                  / sum(r, daref(r,t)) ;
$if      set scapequaldistri     scapr_emit_impactap(impactap,i,v,t)     = sum(r, daref(r,t) * scap_emit_impactap_i(impactap,i,v,r,t))   / sum(r, daref(r,t)) ;
$if      set scapequaldistri     scapr_emit_ap(ap,i,v,t)                 = sum(r, daref(r,t) * scap_emit_ap_i(ap,i,v,r,t))               / sum(r, daref(r,t)) ;
$if      set scapequaldistri     scapr_emit(i,v,t)                       = sum(r, daref(r,t) * scap_emit_i(i,v,r,t))                     / sum(r, daref(r,t)) ;
...
$if      set nogdpgrowth         scap(ap,impactap,r,t)                   = scap_i(ap,impactap,r,t)                   / gdpgrowth(r,t) ;
$if      set nogdpgrowth         scap_emit_impactap(impactap,i,v,r,t)    = scap_emit_impactap_i(impactap,i,v,r,t)    / gdpgrowth(r,t) ;
$if      set nogdpgrowth         scap_emit_ap(ap,i,v,r,t)                = scap_emit_ap_i(ap,i,v,r,t)                / gdpgrowth(r,t) ;
$if      set nogdpgrowth         scap_emit(i,v,r,t)                      = scap_emit_i(i,v,r,t)                      / gdpgrowth(r,t) ;

$if      set scapequalnogdp      scapr(ap,impactap,t)                    = sum(r, daref(r,t) * scap_i(ap,impactap,r,t)                    / gdpgrowth(r,t) )   / sum(r, daref(r,t)) ;
$if      set scapequalnogdp      scapr_emit_impactap(impactap,i,v,t)     = sum(r, daref(r,t) * scap_emit_impactap_i(impactap,i,v,r,t)     / gdpgrowth(r,t) )   / sum(r, daref(r,t)) ;
$if      set scapequalnogdp      scapr_emit_ap(ap,i,v,t)                 = sum(r, daref(r,t) * scap_emit_ap_i(ap,i,v,r,t)                 / gdpgrowth(r,t) )   / sum(r, daref(r,t)) ;
$if      set scapequalnogdp      scapr_emit(i,v,t)                       = sum(r, daref(r,t) * scap_emit_i(i,v,r,t)                       / gdpgrowth(r,t) )   / sum(r, daref(r,t)) ;
...
scc("2015") = scc_int("2015") ;
scc_emit(i,v,r,"2015") = scc_emit_int(i,v,r,"2015") ;
...
$if      set nost        jvrt("Storage_ST",v,r,t)                                                = NO ;
$if      set nolt        jvrt("Storage_LT",v,r,t)                                                = NO ;
...
capacity_ligmin(s,i,v,r,t)       Minimum dispatch of nuclear capacity
...
$if                              set normal      sum(new,                                        IX(new,r,t)             * sum(tv(t,v)$ivrt(new,v,r,t),         capcost(new,v,r)           *  endeffect(new,v,r,t) ))
$if      set storage     $if     set normal    + sum(newj,                                       IG(newj,r,t)            * sum(tv(t,v)$jvrt(newj,v,r,t),       gcapcost(newj,v,r)          * gendeffect(newj,v,r,t) ))
$if      set trans       $if     set normal    + sum((rr,transmission)$tmap(transmission,r,rr),  IT(transmission,r,rr,t) * sum(tv(t,v)$tvrt(transmission,v,t), tcapcost(transmission,r,rr) * tendeffect(transmission,v,t) ))

*                                                Investment costs follow from annuities (%) of borrowed capital (EUR/kW * GW)
$if                              set annui       sum(new,  sum((tt,v)$((tt.val le t.val) and tv(tt,v) and ivrt(new,v,r,tt)),  IX(new,r,tt)  *  capcost(new,v,r)  * deprtime(new,v,r,tt) * annuity(new) * nyrs(t) ) )
$if      set storage     $if     set annui     + sum(newj, sum((tt,v)$((tt.val le t.val) and tv(tt,v) and jvrt(newj,v,r,tt)), IG(newj,r,tt) * gcapcost(newj,v,r) * gdeprtime(newj,v,r,tt) * gannuity(newj,v) * nyrs(t) ) )
$if      set trans       $if     set annui     + sum((rr,transmission)$tmap(transmission,r,rr), sum((tt,v)$((tt.val le t.val) and tv(tt,v) and tvrt(transmission,v,tt)), IT(transmission,r,rr,t) * tcapcost(transmission,r,rr) * tdeprtime(transmission,v,tt) * tannuity(transmission) * nyrs(t) ) )

*                                                Investment costs follow from WACC (%) of capital stock (EUR/kW * GW)
$if                              set ccost       sum(new, sum((tt,v)$((tt.val le t.val) and tv(tt,v) and ivrt(new,v,r,tt)), IX(new,r,tt) * capcost(new,v,r) * deprtime(new,v,r,tt) * drate * nyrs(t) ) )
$if      set storage     $if     set ccost     + sum(newj, sum((tt,v)$((tt.val le t.val) and tv(tt,v) and jvrt(newj,v,r,tt)), IG(newj,r,tt) * gcapcost(newj,v,r) * gdeprtime(newj,v,r,tt) * drate * nyrs(t) ) )
$if      set trans       $if     set ccost     + sum((rr,transmission)$tmap(transmission,r,rr), sum((tt,v)$((tt.val le t.val) and tv(tt,v) and tvrt(transmission,v,tt)), IT(transmission,r,rr,t) * tcapcost(transmission,r,rr) * tdeprtime(transmission,v,tt) * drate * nyrs(t) ) )
                 )
                 !! end investment cost
...
$if      set scn         + dfact_scc(t)  / dfact(t) * 1e-3 * sum(ivrt(i,v,r,t), scn_emit(i,v,r,t)) * sum(s, X(s,i,v,r,t) * hours(s) ))
*                        Social cost of air pollution (EUR per MWh) from generation (GWh)
                         + dfact_scap(t) / dfact(t) * 1e-3 * sum(ivrt(i,v,r,t), scap_emit(i,v,r,t) * sum(s, X(s,i,v,r,t) * hours(s) ))
*                        Social cost of carbon (EUR per MWh) from generation (GWh)
                         + dfact_scc(t)  / dfact(t) * 1e-3 * sum(ivrt(i,v,r,t), scc_emit(i,v,r,t)  * sum(s, X(s,i,v,r,t) * hours(s) ))
...
capacity_nucmin(s,ivrt(i,v,r,t))$(mindisp(i,v,r) and tmyopic(t) and sameas(i,"Nuclear"))..
...
* Minimum dispatch of nuclear (this condition is deactivitated if not "ligmin=yes")
capacity_ligmin(s,ivrt(i,v,r,t))$(mindisp(i,v,r) and tmyopic(t) and sameas(i,"Lignite"))..
                 X(s,i,v,r,t) =g=  XCS(s,i,v,r,t) * mindisp(i,v,r) * (1 + (reliability(i,v,r)-1)$reliability(i,v,r)) * (1 + (af(s,i,r)-1)$af(s,i,r)) * (1 + (vrsc(s,i,r)-1)$vrsc(s,i,r)) ;
