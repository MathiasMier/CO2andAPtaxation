...
** Preferences
        ...
        prstp    Initial rate of social time preference per year   /.004  /
** Population and technology
        gama     Capital elasticity in production function        /.300    /
* Changed from 7403 to 7339
...
** Changed to 86.138
        q0       Initial world gross output 2015 (trill 2015 USD) /86.138  /
        k0       Initial capital value 2015 (trill 2015 USD)      /223     /
...
** Changed from 2.6 to 6.25 Gt
        eland0   Carbon emissions from land 2015 (GtCO2 per year)     / 6.25 /
        deland   Decline rate of land emissions (per period)          / .115   /
** Changed from 35.85 to 35.21 Gt
        e0       Industrial emissions 2015 (GtCO2 per year)           /35.21   /
...
** Deleted "after 2150" since control rate might be lower already before, changed from 1.2 to 1
...
** Added
        popadj1      Total population adjustment factor           /0.99135197/
        popadj2      Total population adjustment factor           /0.99477533/
        popadj3      Total population adjustment factor           /0.99582697/
        popadj4      Total population adjustment factor           /0.99715122/
        popadj5      Total population adjustment factor           /0.99816488/
        popadj6      Total population adjustment factor           /0.99900418/
        popadj7      Total population adjustment factor           /0.99955463/
        popadj8      Total population adjustment factor           /0.99956651/

** Added
        aladj0      Total factor productivity adjustment factor  /0.82539033/
        aladj1      Total factor productivity adjustment factor  /1.00000650/
        aladj2      Total factor productivity adjustment factor  /1.00199854/
        aladj3      Total factor productivity adjustment factor  /0.99609560/
        aladj4      Total factor productivity adjustment factor  /0.99266810/
        aladj5      Total factor productivity adjustment factor  /0.99092965/
        aladj6      Total factor productivity adjustment factor  /0.98907118/
        aladj7      Total factor productivity adjustment factor  /0.98225792/
        aladj8      Total factor productivity adjustment factor  /0.99504452/ ;
...
**Added
        ladj(t)       Total populaqtion adjustment factor
        al(t)         Level of total factor productivity
**Added
        aladj(t)      Total factor productivity adjustment factor
...
**Added
        ...
        ladj("7") = popadj7;
        ladj("8") = popadj8;
        loop(t$(t.val ge 9), ladj(t) = popadj8; );

        l("1") = ladj("1") * pop0;
*        loop(t, l(t+1) = l(t););
        loop(t, l(t+1) = ladj(t+1) * l(t) * (popasym/l(t))**popadj ;);
...
**Added
        ...
        aladj("7") = aladj7;
        aladj("8") = aladj8;
        loop(t$(t.val ge 9), aladj(t)=aladj8; );
...
VARIABLES
        ...
        C(t)            Consumption (trillions 2015 US dollars per year)
        K(t)            Capital stock (trillions 2015 US dollars)
        CPC(t)          Per capita consumption (thousands 2015 USD per year)
        I(t)            Investment (trillions 2015 USD per year)
        S(t)            Gross savings rate as fraction of gross world product
        RI(t)           Real interest rate (per annum)
        Y(t)            Gross world product net of abatement and damages (trillions 2015 USD per year)
        YGROSS(t)       Gross world product GROSS of abatement and damages (trillions 2015 USD per year)
        YNET(t)         Output net of damages equation (trillions 2015 USD per year)
        DAMAGES(t)      Damages (trillions 2015 USD per year)
        DAMFRAC(t)      Damages as fraction of gross output
        ABATECOST(t)    Cost of emissions reductions  (trillions 2015 USD per year)
        MCABATE(t)      Marginal cost of abatement (2015$ per ton CO2)
        CCA(t)          Cumulative industrial carbon emissions (GTC)
        CCATOT(t)       Total carbon emissions (GtC)
        PERIODU(t)      One period utility function
        CPRICE(t)       Carbon price (2015$ per ton of CO2)
...
*Economic variables
**Added a aladj factor to scale DICE GDP to real world GDP
 ygrosseq(t)..        YGROSS(t)      =E= (aladj(t) * aladj0 * al(t)*(l(t)/1000)**(1-GAMA))*(K(t)**GAMA);
...
cl(t) = 1000 * C.L(t)/L(t) ;
clgrowth = (cl("8")/cl("1"))**(1/(2050-2015))-1 ;
sdr = prstp + clgrowth * elasmu ;
taxdiff(t) = (1/(1+sdr))**((t.val - 1)*5) / (1/(1+0.07))**((t.val - 1)*5) ;
tax(t) = taxdiff(t) * scc(t) ;
;

execute_unload "DICE-2016R2_adjusted_final_new.gdx" rr, scc, eeq.M, cc.M, cprice.L, l, K.L, I.L, S.L, E.L, EIND.L, etree, C.L, YGROSS.L, YNET.L, Y.L, DAMAGES.L, DAMFRAC.L, MIU.L, TATM.L, TOCEAN.L, cl, clgrowth, sdr, taxdiff, tax;
